...
fn create_cake_size(pool: sqlx::PgPool) -> Result<(), Error> {
    ...
    let (status, Json(new_size)) = create_size(state.clone(), Json(payload)).await?;

    assert_eq!(status, StatusCode::CREATED);

    let size = sqlx::query_as::<_, CakeSize>("SELECT * FROM cake_sizes WHERE id = $1")
        .bind(new_size.id)
...
fn delete_cake_size(pool: sqlx::PgPool) -> Result<(), Error> {
    ...
    let id = Path(size.id);
    delete_size(state.clone(), id).await?;
